...
def main():
    ...
    test_argv = [
        ...
        '--postnovo_seqs_info',
        'C:\\Users\\Samuel\\Downloads\\postnovo_blastseqs_test_121117\\postnovo_seqs_info.tsv',
        '--iodir',
        'C:\\Users\\Samuel\\Downloads\\postnovo_blastseqs_test_121117'
        ]
    ...
    if args.command == 'make_emapper_input':
        ...
        analyze_emapper_output(
            args, sampled_table, lower_prob_table, high_prob_taxa_table, eggnog_fasta_basenames
            )
...
def parse_args(test_argv = None):
    ...
    parser_analyze_emapper_output.add_argument(
        '--postnovo_seqs_info',
        help='path to postnovo_seqs_info.tsv, the file of query sequence info generated by postnovo'
        )
    parser_analyze_emapper_output.add_argument(
        '--iodir',
...
def make_emapper_input(args):
    ...
    if args.redo_without_search:
        ...
        db_out_path_dict = run_blast(args.blast_dbs, db_names, split_blast_fasta_fp_list, args.blastp)
    ...
    eggnog_fasta_basenames = write_eggnog_fasta_files(sampled_out_table, out_dir, fasta_filename_prefix, args.emapper_tool)
...
def assign_taxonomy(align_table):
    ...
    groupby_seq_tables = [groupby_seq_table for _, groupby_seq_table in align_table.groupby('seq_number')]
    ...
    for groupby_seq_table in groupby_seq_tables:
        ...
        representative_hits = groupby_seq_table.drop_duplicates(subset=['taxid'])
    ...
    # This is not a table so much as a set of ordered cols
    ...
    for rank in search_ranks:
        ...
        # Sort the taxa in descending order of counts (often 'None' will be the most abundant taxon)
...
def write_eggnog_fasta_files(sampled_out_table, out_dir, fasta_filename_prefix, emapper_tool):
    ...
    for superkingdom in superkingdoms:
        ...
        if len(superkingdom_out_table) > 0:
            seq_numbers = [str(seq_number) for seq_number in superkingdom_out_table['seq_number'].tolist()]
            ...
            if emapper_tool == 'diamond':
                path = os.path.join(
                    out_dir, os.path.splitext(generic_fasta_filename)[0] + '.' + superkingdom.lower() + '.faa'
                    )
                eggnog_fasta_basenames.append(os.path.basename(path))
...
def analyze_emapper_output(args, sampled_table, lower_prob_table, high_prob_taxa_table, eggnog_fasta_basenames):
    ...
    reported_df = condense_hits(conserv_func_df, lower_prob_table, high_prob_taxa_table, args.postnovo_seqs_info)
    reported_df.to_csv(
...
def find_coherent_hits(eggnog_df, sampled_table):
    ...
    # Need a table of all seq number + hit number
    # Groupby seq number, make sorted list of hit number
    # A list of hits must have matching annotation strings to be called conserved

    conserv_func_df_list = []
    eggnog_df_groups = eggnog_df.groupby('seq_number')
    sampled_table_groups = sampled_table.groupby('seq_number')
    for seq_number in list(set(eggnog_df['seq_number'])):
        eggnog_df_group = eggnog_df_groups.get_group(seq_number)
        sampled_table_group = sampled_table_groups.get_group(seq_number)
...
def condense_hits(conserv_func_df, lower_prob_table, high_prob_taxa_table, seq_info_path):
    ...
    seq_numbers = list(set(conserv_func_df['seq_number'].tolist()))
    ...
    parsed_conserv_func_df['seq_number'] = seq_numbers
    conserv_func_df_groups = conserv_func_df.groupby('seq_number')
    # Loop through each group
    for seq_number in seq_numbers:
        conserv_func_df_group = conserv_func_df_groups.get_group(seq_number)
        ...
        list_of_lists = conserv_func_df_group['bigg reactions'].apply(lambda x: x.split(',')).tolist()
        l = [i for sublist in list_of_lists for i in sublist]
    ...
    lower_prob_table.set_index('seq_number', inplace=True)
    seq_numbers = sorted(list(set(seq_numbers).intersection(set(lower_prob_table.index.tolist()))))
    lower_prob_table = lower_prob_table.loc[seq_numbers]
    lower_prob_table.reset_index(inplace=True)
    low_prob_taxa_table, low_prob_taxa_count_table = \
        assign_taxonomy(lower_prob_table)
    ...
    reported_df = parsed_conserv_func_df.merge(taxa_table, how='inner', on='seq_number')
    ...
    reported_df = reported_df[seq_info_hdr + retained_eggnog_output_headers + search_ranks]
