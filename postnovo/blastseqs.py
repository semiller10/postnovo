...
def main(test_argv=None):
    global from_postnovo, id_type, local_eggnog, out_dir
    ...
    if args.command == 'make_emapper_input':
        ...
        if 'scan_list' in sampled_df.columns:
            from_postnovo = True
            id_type = 'scan_list'
...
def make_query_files(args):
    ...
    if args.seq_table != None:
        fasta_list = make_fasta_input(args.seq_table)
    elif args.fasta_from_postnovo != None:
        fasta_list = open(args.fasta_from_postnovo, 'r').readlines()
        fasta_input_fp = args.fasta_from_postnovo
    elif args.fasta_not_from_postnovo != None:
        fasta_list = open(args.fasta_not_from_postnovo, 'r').readlines()
        fasta_input_fp = args.fasta_not_from_postnovo
...
def parse_args(test_argv = None):
    ...
    make_emapper_input_seq_source_group.add_argument(
        '--fasta_from_postnovo',
        help='path to fasta file generated by postnovo'
        )
    make_emapper_input_seq_source_group.add_argument(
        '--fasta_not_from_postnovo',
        help='path to fasta file NOT generated by postnovo'
        )
    make_emapper_input_seq_source_group.add_argument(
        '--seq_table',
        help=('csv table with two columns of seq headers and seqs: '
              'all PTM symbols must be removed from seqs, '
              'columns must not have headers')
        )
    ...
    parser_make_emapper_input.add_argument(
        ...
        help=(
            ...
            'Databases should be constructed from fasta files '
            'with all Ile amino acids converted to Leu. '
...
def set_global_vars(args):
    ...
    global cores, from_postnovo, id_type

    if args.command == 'make_emapper_input':
        cores = args.cores
        if args.fasta_from_postnovo != None:
            from_postnovo = True
            id_type = 'scan_list'
...
def check_args(parser, args):
    ...
    if args.command == 'make_emapper_input':
        ...
        if (args.fasta_from_postnovo == None and 
            args.fasta_not_from_postnovo == None and
            args.seq_table == None):
            parser.error('fasta or seq table input must be provided')
        if args.fasta_from_postnovo != None:
            if not os.path.exists(args.fasta_from_postnovo):
                parser.error(args.fasta_from_postnovo + ' does not exist')
        elif args.fasta_not_from_postnovo != None:
            if not os.path.exists(args.fasta_not_from_postnovo):
                parser.error(args.fasta_not_from_postnovo + ' does not exist')
        else:
            if not os.path.exists(args.seq_table):
                parser.error(args.seq_table + ' does not exist')
...
def run_blast(db_fp_list, fasta_fp_list, blastp_fp):
    ...
    num_threads = int(cores / max_processes_allowed)
    ...
    while len(processes) > 0 or instances_started == 0:
        while len(processes) < max_processes_allowed and instances_started < len(fasta_fp_list):
