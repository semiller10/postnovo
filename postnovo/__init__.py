...
def main():
    ...
    save_pkl_objects(config.iodir[0], **{'alg_basename_dfs_dict': alg_basename_dfs_dict})
    #alg_basename_dfs_dict = load_pkl_objects(config.iodir[0], 'alg_basename_dfs_dict')
    ## example:
    ...
    save_pkl_objects(config.iodir[0], **{'consensus_prediction_df': prediction_df})
    #prediction_df = load_pkl_objects(config.iodir[0], 'consensus_prediction_df')

    prediction_df = masstol.update_prediction_df(prediction_df)
    save_pkl_objects(config.iodir[0], **{'mass_tol_prediction_df': prediction_df})
    #prediction_df = load_pkl_objects(config.iodir[0], 'mass_tol_prediction_df')

    prediction_df = interspec.update_prediction_df(prediction_df)
    save_pkl_objects(config.iodir[0], **{'interspec_prediction_df': prediction_df})
    #prediction_df = load_pkl_objects(config.iodir[0], 'interspec_prediction_df')
