...
# These fields should apply to emapper output when either DIAMOND or HMMER is used
...
confident_evalue = 0.01
# The number of hits of equivalent strength to retain from DIAMOND and BLAST+ alignment tables
hit_sample_size = 5
...
def main():
    ...
    #    'toolik_13_2_1_1', 
    #    '--postnovo_seqs', 
    #    'C:\\Users\\Samuel\\Downloads\\postnovo_blastseqs_make_emapper_input_test\\postnovo_seqs.faa', 
    #    '--postnovo_seqs_info', 
    #    'C:\\Users\\Samuel\\Downloads\\postnovo_blastseqs_make_emapper_input_test\\postnovo_seqs_info.tsv', 
    #    '--diamond', 
    #    'bob', 
    #    '--diamond_db', 
    #    'bob', 
    ...
    #    '--iodir',
    #    'C:\\Users\\Samuel\\Downloads\\postnovo_blastseqs_analyze_emapper_output_test'
    #    ]
    ...
    if args.command == 'make_emapper_input':
        ...
        postnovo_seqs_info = os.path.join(args.iodir, 'postnovo_seqs_info.tsv')
        sampled_table = pd.read_csv(
            os.path.join(out_dir, 'sampled_table.csv')
            )
        lower_prob_table = pd.read_csv(
            os.path.join(out_dir, 'lower_prob_table.csv')
            )
        high_prob_taxa_table = pd.read_csv(
            os.path.join(out_dir, 'high_prob_taxa_table.csv')
            )
        with open(os.path.join(out_dir, 'eggnog_fasta_basenames.txt')) as handle:
            eggnog_fasta_basenames = [basename.rstrip() for basename in handle.readlines()]
        ...
        analyze_emapper_output(
            ...
            sampled_table, 
            lower_prob_table, 
            high_prob_taxa_table, 
            eggnog_fasta_basenames
        )
...
def parse_args(test_argv = None):
    ...
    parser_make_emapper_input.add_argument(
        '--dataset_name',
        help='dataset name to use in emapper input filename'
    )
    ...
    parser_make_emapper_input.add_argument(
        '--taxonmap',
        help='filepath to gzipped accession2taxid'
    )
    parser_make_emapper_input.add_argument(
        '--blastp',
        help=(
    ...
    parser_make_emapper_input.add_argument(
        '--emapper_tool',
        choices=['diamond', 'hmmer'],
        default='diamond',
        help=(
            'The search tool to use in eggNOG-mapper: '
            'diamond is faster but less sensitive than hmmer. '
            'The main consequence of this choice '
            'is the number of fasta files generated: '
            'hmmer can only handle 5,000 seqs per file.'
            )
    )
    parser_make_emapper_input.add_argument(
        '--cores',
    ...
    parser_analyze_emapper_output = subparsers.add_parser(
        'analyze_emapper_output',
        help='analyze eggNOG-mapper annotations'
    )
    parser_analyze_emapper_output.add_argument(
        '--iodir',
        help=('directory containing emapper annotation output, '
              'sampled_table.csv, '
              'lower_prob_table.csv, '
              'high_prob_taxa_table.csv, '
              'and eggnog_fasta_basenames.txt (all produced by make_emapper_input step), '
...
def check_args(parser, args):
    ...
    if args.command == 'make_emapper_input':
        ...
        if args.redo_without_search == False:
            ...
            if args.taxonmap == None:
                parser.error('gzipped accession2taxid filepath must be provided')
            if not os.path.exists(args.taxonmap):
                parser.error(args.taxonmap + ' does not exist')
            if os.path.splitext(args.taxonmap)[1] != '.gz':
                parser.error(args.taxonmap + ' must be gzipped, with extension .gz')
            ...
            if args.blast_dbs == None:
                parser.error('BLAST+ database path(s) must be provided')
            ...
            if args.blast_dbs == None:
                parser.error(
                    'BLAST+ database path(s) or name(s) (e.g., "blastp-short") must be provided '
                    'to retrieve the BLAST+ output that was generated '
                    'and should now be in the same directory as the postnovo_seqs files'
                )
    ...
    # analyze_emapper_output subcommand
    elif args.command == 'analyze_emapper_output':
        if not os.path.exists(args.iodir):
            parser.error(args.iodir + ' does not exist')
        if (
            not os.path.exists(os.path.join(args.iodir, 'sampled_table.csv')) or 
            not os.path.exists(os.path.join(args.iodir, 'lower_prob_table.csv')) or
            not os.path.exists(os.path.join(args.iodir, 'high_prob_taxa_table.csv')) or 
            not os.path.exists(os.path.join(args.iodir, 'eggnog_fasta_basenames.txt'))
            ):
            parser.error(
                'the following files must be present in ' + 
                args.iodir + 
                ': postnovo_seqs_info.tsv, '
                'sampled_table.csv, '
                'lower_prob_table.csv, '
                'high_prob_taxa_table.csv, '
                'eggnog_fasta_basenames.txt'
            )
        with open(os.path.join(args.iodir, 'eggnog_fasta_basenames.txt')) as handle:
            eggnog_fasta_basenames = [basename.rstrip() for basename in handle.readlines()]
            for basename in eggnog_fasta_basenames:
                if not os.path.exists(os.path.join(args.iodir, basename)):
                    parser.error(
                        'the following files must be present in ' + 
                        args.iodir + 
                        '\n '.join(eggnog_fasta_basenames)
                    )
...
def make_emapper_input(args):
    ...
    if args.redo_without_search:
        iodir = os.path.dirname(args.postnovo_seqs)
        split_blast_fasta_fp_list = glob.glob(
            os.path.join(iodir, 'postnovo_seqs_*.blastp-short.faa')
            )
        ...
        split_blast_fasta_fp_list = split_blast_fasta(
            blast_fasta_list, args.max_seqs_per_blast_instance
            )

    db_names = [os.path.basename(blast_db_fp) for blast_db_fp in args.blast_dbs]
    # If DIAMOND and BLAST+ have already been run, but the output has not been analyzed,
    # avoid running the programs again and find the paths of the expected output files
    if args.redo_without_search:
        # The output paths from each db are keyed by each db name
        db_out_path_dict = OrderedDict.fromkeys(db_names)
        for db_name in db_names:
            db_out_path_dict[db_name] = []
            for split_blast_fasta_fp in split_blast_fasta_fp_list:
                db_out_path_dict[db_name].append(
                    os.path.splitext(split_blast_fasta_fp)[0] + '_' + db_name + '.out'
                )
    else:
        subprocess.call([
            args.diamond,
            ...
            '--db', args.diamond_db,
            '--query', diamond_fasta_fp,
            '--taxonmap', args.taxonmap,
            '--more-sensitive',
            '--out', os.path.splitext(diamond_fasta_fp)[0] + '.out',
        ...
        db_out_path_dict = run_blast(
            args.blast_dbs, 
            db_names, 
            split_blast_fasta_fp_list, 
            args.blastp
            )
    ...
    for db_name in db_names:
        ...
        for partial_blast_out_path in db_out_path_dict[db_name]:
            partial_blast_out_table = pd.read_csv(
                partial_blast_out_path, sep='\t', header=None, names=blast_out_hdr
                )
        ...
        blast_out_table['hit_number'] = make_hit_number_list(
            blast_out_table['seq_number'].tolist()
            )
    ...
    diamond_out_table = pd.read_csv(
        os.path.splitext(fasta_input_fp)[0] + '.diamond.out', 
        sep='\t', 
        header=None, 
        names=diamond_out_hdr
        )
    ...
    diamond_out_table['hit_number'] = make_hit_number_list(
        diamond_out_table['seq_number'].tolist()
        )
    ...
    # Find taxids for accessions that could not be mapped by accession2taxid file
    missing_taxid_subtable = diamond_out_table[pd.isnull(diamond_out_table['taxid'])]
    missing_taxid_indices = missing_taxid_subtable.index.tolist()
    accessions = missing_taxid_subtable['accession'].tolist()
    one_percent_number_accessions = len(accessions) / 100 / cores
    ...
    #    procedure_str='Missing DIAMOND taxid retrieval progress: ',
    #    one_percent_total_count=one_percent_number_accessions * cores
    #    )
    ...
    print_percent_progress_fn = partial(
        ...
        procedure_str='Missing DIAMOND taxid retrieval progress: ',
        one_percent_total_count=one_percent_number_accessions,
        cores=cores)
    single_var_get_taxid = partial(
        get_taxid, print_percent_progress_fn=print_percent_progress_fn
        )
    multiprocessing_pool = Pool(cores)
    taxids = multiprocessing_pool.map(single_var_get_taxid, accessions)
    multiprocessing_pool.close()
    multiprocessing_pool.join()

    # Set the NaN taxid values to the recovered taxids
    diamond_out_table.loc[missing_taxid_indices, 'taxid'] = taxids
    ...
    if args.intermediate_files:
        align_out_table.to_csv(
            os.path.join(out_dir, 'alignments_table.csv'), index=False
        )
    ...
    high_prob_out_table, lower_prob_out_table, filtered_out_table = filter_align_out_table(
        align_out_table
        )

    if args.intermediate_files:
        high_prob_out_table.to_csv(
            os.path.join(out_dir, 'high_prob_alignments_table.csv'), index=False
        )
        lower_prob_out_table.to_csv(
            os.path.join(out_dir, 'lower_prob_alignments_table.csv'), index=False
        )
        filtered_out_table.to_csv(
            os.path.join(out_dir, 'filtered_alignments_table.csv'), index=False
        )
    ...
    if args.intermediate_files:
        high_prob_out_table.to_csv(
            os.path.join(out_dir, 'high_prob_alignments_with_taxonomy_table.csv'), index=False
        )
        lower_prob_out_table.to_csv(
            os.path.join(out_dir, 'lower_prob_alignments_with_taxonomy_table.csv'), index=False
        )
        filtered_out_table.to_csv(
            os.path.join(out_dir, 'filtered_alignments_with_taxonomy_table.csv'), index=False
        )
    ...
    high_prob_taxa_table, high_prob_taxa_count_table = assign_taxonomy(
        high_prob_out_table, taxon_assignment_threshold=0.9
        )
    ...
    filtered_out_table, lower_prob_out_table = filter_lower_prob_taxa(
        high_prob_taxa_count_table, high_prob_out_table, lower_prob_out_table
        )

    if args.intermediate_files:
        filtered_out_table.to_csv(
            os.path.join(out_dir, 'filtered_table.csv'), index=False
        )
    lower_prob_out_table.to_csv(
        os.path.join(out_dir, 'lower_prob_table.csv'), index=False
    )

    # Multiple hits per query seq can be retained at this stage
    # Each set of hits must have uniform functional annotations
    # When more than <hit_sample_size> hits are considered, evenly sample <hit_sample_size> hits
    print('evenly sample ' + str(hit_sample_size) + ' hits')
    sampled_out_table = filtered_out_table.groupby('seq_number').apply(sample_hits)
    sampled_out_table.to_csv(
        os.path.join(out_dir, 'sampled_table.csv'), index=False
    )
    ...
    sampled_out_table['full_subject_seq'] = get_full_subject_seqs(
        sampled_out_table['accession'].tolist()
        )
    sampled_out_table.to_csv(
        os.path.join(out_dir, 'full_seq_table.csv'), index=False
        )
    #sampled_out_table = pd.read_csv(
    #    os.path.join(out_dir, 'full_seq_table.csv'), header=0
    #    )
    ...
    eggnog_fasta_basenames = write_eggnog_fasta_files(
        sampled_out_table, 
        out_dir, 
        args.emapper_tool, 
        args.dataset_name
        )
    # Write a list of the fasta files 
    # to ensure they are all accounted for in postnovo.blastseqs analyze_emapper_output
    with open(os.path.join(out_dir, 'eggnog_fasta_basenames.txt'), 'w') as handle:
        for basename in eggnog_fasta_basenames:
...
def run_blast(db_fp_list, db_names, fasta_fp_list, blastp_fp):
    ...
    # The list of all BLAST output paths is split into sublists for each db
    db_out_path_dict = OrderedDict().fromkeys(db_names)
    for db_names in db_names:
        db_out_path_dict[db_name] = []
        for out_path in all_out_paths:
            if db_name in out_path:
                db_out_path_dict[db_name].append(out_path)

    return db_out_path_dict
...
def get_taxids(blast_out_table):
    ...
    groupby_taxon_tables = blast_out_table.groupby('taxon')
    taxon_out_tables = [taxon_out_table for _, taxon_out_table in blast_out_table.groupby('taxon')]

    ## Single process
    #representative_accessions = []
    #for taxon_out_table in [taxon_out_table for _, taxon_out_table in groupby_taxon_tables]:
    ...
    representative_accessions = multiprocessing_pool.map(
        get_representative_accession, [taxon_out_table for _, taxon_out_table in groupby_taxon_tables]
        )
    multiprocessing_pool.close()
    multiprocessing_pool.join()
    ...
    # The order of taxids corresponds to the order of taxa (groups) in the groupby object
    taxid_dict = OrderedDict().fromkeys([taxon for taxon, _ in groupby_taxon_tables])
...
def assign_taxonomy(align_table, group_col='seq_number', taxon_assignment_threshold=1):

    # Make a separate table for each query seq in the table of alignment results
    groupby_seq_tables = [
        groupby_seq_table for _, groupby_seq_table in align_table.groupby(group_col)
        ]
    # Rows of parsimonious taxonomic assignments, each corresponding to a query seq
    taxa_assignments = []
    for groupby_seq_table in groupby_seq_tables:
        # High-prob output table (make_emapper_input), consisting of BLAST output,
        # can have different taxids for different hits
        # Only consider a subset of hits representing a unique set of taxids
        # Low-prob and high-prob taxa tables (analyze_emapper_output), grouped by scan,
        # do not have a taxid column, as there is only one taxid per row
        if group_col == 'seq_number':
            groupby_seq_table = groupby_seq_table.drop_duplicates(subset=['taxid'])
        # Loop through each taxonomic rank to be considered
        for rank_index, rank in enumerate(search_ranks):
            # At the given rank, find the most common Linnean name
            most_common_name = Counter(groupby_seq_table[rank]).most_common(1)[0]
            # Proceed if the lineage was assigned at the rank
            if most_common_name[0] != 'None' and pd.notnull(most_common_name[0]):
                # Proceed if the Linnean name applies to at least a minimum proportion of taxids
                if most_common_name[1] >= taxon_assignment_threshold * len(groupby_seq_table):
                    # make_emapper_input:
                    # Take as the best hit
                    # the one with the lowest e-value and the Linnean name (first in the table)
                    if group_col == 'seq_number':
                        representative_hit = groupby_seq_table.ix[
                            groupby_seq_table[
                                groupby_seq_table[rank] == most_common_name[0]
                    ...
                    # analyze_emapper_output:
                    # Consolidate best_predicts_from and also_has_predicts_from into a single entry
                    elif group_col == 'scan_list':
                        representative_row = groupby_seq_table.ix[
                            groupby_seq_table[
                                groupby_seq_table[rank] == most_common_name[0]
                                ][rank].first_valid_index()
                            ]
                        all_best_predicts_from = []
                        for best_predicts_from in groupby_seq_table['best_predicts_from'].tolist():
                            if pd.notnull(best_predicts_from):
                                all_best_predicts_from += best_predicts_from.split(',')
                        all_best_predicts_from = ','.join(list(set(all_best_predicts_from)))
                        all_also_contains_predicts_from = []
                        for also_contains_predicts_from in groupby_seq_table[
                            'also_contains_predicts_from'
                            ].tolist():
                            if pd.notnull(also_contains_predicts_from):
                                all_also_contains_predicts_from += \
                                    also_contains_predicts_from.split(',')
                        all_also_contains_predicts_from = ','.join(
                            list(set(all_also_contains_predicts_from))
                            )
                        taxa_assignments.append(
                            [
                                representative_row['scan_list'], 
                                all_best_predicts_from, 
                                all_also_contains_predicts_from
                            ] + 
                            rank_index * ['None'] + 
        ...
        # due to discovery of a "finest-grained" taxonomic name shared in common among the hits,
        # record that no common taxonomy could be found for the seq (no name at any rank)
        else:
            if group_col == 'seq_number':
                representative_hit = groupby_seq_table.iloc[0]
                taxa_assignments.append(
                    representative_hit[seq_info_hdr].tolist() + 
                    len(search_ranks) * ['None']
                )
            elif group_col == 'scan_list':
                representative_row = groupby_seq_table.iloc[0]
                all_best_predicts_from = []
                for best_predicts_from in groupby_seq_table['best_predicts_from'].tolist():
                    if pd.notnull(best_predicts_from):
                        all_best_predicts_from += best_predicts_from.split(',')
    ...
    if group_col == 'seq_number':
        ...
        taxa_table = pd.DataFrame(
            taxa_assignments, 
            columns=['scan_list', 'best_predicts_from', 'also_contains_predicts_from'] + \
                search_ranks
                )

    if group_col == 'seq_number':
        # Determine the number of assignments to each taxonomic name at each rank
        # This produces a set of paired, ordered cols
        ...
        for rank in search_ranks:
            ...
            rank_taxa_count_table = pd.concat([
            ...
            # Sort the taxa in descending order of counts
            # (often 'None' will be the most abundant taxon)
            rank_taxa_count_table.sort_values(rank + ' count', ascending=False, inplace=True)
            taxa_count_table = pd.concat([
                taxa_count_table,
                rank_taxa_count_table
                ], axis=1)
...
def filter_lower_prob_taxa(high_prob_taxa_count_table, high_prob_out_table, lower_prob_out_table):
    '''
    ...
    for rank in profile_ranks:
        ...
        # Also check for a rank name of "None," when a lineage is not defined at a rank
        rank_table = high_prob_taxa_count_table[
            (high_prob_taxa_count_table[rank + ' name'] != 'None') & 
            (pd.notnull(high_prob_taxa_count_table[rank + ' name']))
            ][[rank + ' name', rank + ' count']]
        high_prob_name_list = rank_table[rank + ' name'].tolist()
        high_prob_count_list = rank_table[rank + ' count'].tolist()
        count_dict = OrderedDict([
            (name, high_prob_count_list[i]) for i, name in enumerate(high_prob_name_list)
            ])
...
def sample_hits(seq_retained_hits):
    ...
    if number_top_hits <= hit_sample_size:
        return top_hits
    else:
        hit_indices = list(range(number_top_hits))
        rounded_quotient, remainder = divmod(len(hit_indices), hit_sample_size)
        sample_indices = [
            hit_indices[i * rounded_quotient + min(i, remainder)] for i in range(hit_sample_size)
            ]
...
def write_eggnog_fasta_files(
    ...
    for superkingdom in superkingdoms:
        ...
        superkingdom_out_table = sampled_out_table[
            sampled_out_table['superkingdom'] == superkingdom
            ]
        # Only write to file if there is data for the superkingdom
        if len(superkingdom_out_table) > 0:
            seq_numbers = [
                str(seq_number) for seq_number in superkingdom_out_table['seq_number'].tolist()
                ]
            # REMOVE
            hit_numbers = []
            for hit_number in superkingdom_out_table['hit_number'].tolist():
                hit_numbers.append(str(hit_number))
                print(str(hit_number))
            #hit_numbers = [
            #    str(hit_number) for hit_number in superkingdom_out_table['hit_number'].tolist()
            #    ]
            ...
            if emapper_tool == 'diamond':
                # It is only practical to run online emapper via DIAMOND with 50,000 seqs per file
                # due to upload limitations
                fasta_seq_count_limit = 50000
            elif emapper_tool == 'hmmer':
                # Online emapper via HMMER allows up to 5,000 seqs per file
                fasta_seq_count_limit = 5000

            # Keep count of the next seq to go into a fasta file from the full list of seqs
            hit_index = 0
            # Loop through each batch of seqs
            for i in range(len(fasta_headers) // fasta_seq_count_limit + 1):
                split_fasta_basename = 'postnovo_seqs_emapper.' + \
                    superkingdom.lower() + '.' + dataset_name + '.' + str(i) + '.faa'
                eggnog_fasta_basenames.append(split_fasta_basename)
                path = os.path.join(out_dir, split_fasta_basename)
                ...
                with open(path, 'w') as handle:
                    ...
                    for j, header in enumerate(fasta_headers[hit_index:]):
                        ...
                        # Stop writing to the file when the seq limit is reached
                        if j + 1 == fasta_seq_count_limit:
                            # Calculate the index of the first seq in the next file
                            hit_index = fasta_seq_count_limit * (i + 1)
                            break

    # Return the list of fasta filenames
    return eggnog_fasta_basenames

def analyze_emapper_output(
    args, 
    postnovo_seqs_info, 
    sampled_table, 
    lower_prob_table, 
    ...
    eggnog_out_fp_list = [
        os.path.join(args.iodir, basename + '.emapper.annotations') for basename in eggnog_fasta_basenames
        ]

    eggnog_df = parse_emapper_output(eggnog_out_fp_list)
    conserv_func_df = find_coherent_hits(eggnog_df, sampled_table)
    reported_df = condense_hits(conserv_func_df, lower_prob_table, high_prob_taxa_table, postnovo_seqs_info)
    reported_df.to_csv(
...
def parse_emapper_output(eggnog_out_fp_list):
    ...
    # Split header into two cols for seq number and hit number
    seq_numbers = eggnog_df['query'].apply(
        lambda x: int(x.split('(seq_number)')[1].split('(hit_number)')[0])
        ).tolist()
    hit_numbers = eggnog_df['query'].apply(
        lambda x: int(x.split('(hit_number)')[1])
        ).tolist()
    eggnog_df.drop('query', axis=1, inplace=True)
    eggnog_df['seq_number'] = seq_numbers
    eggnog_df['hit_number'] = hit_numbers

    return eggnog_df

def find_coherent_hits(eggnog_df, sampled_table):

    eggnog_df = eggnog_df.merge(
        sampled_table[['seq_number', 'hit_number', 'scan_list']], 
        how='left', 
        on=['seq_number', 'hit_number']
        )
    # A list of hits to a BLAST query 
    # must have identical eggNOG annotation strings in order to be called conserved
    conserv_func_df_list = []
    eggnog_df_groups = eggnog_df.groupby('scan_list')
    sampled_table_groups = sampled_table.groupby('scan_list')
    for scan_list in list(set(eggnog_df['scan_list'])):
        eggnog_df_group = eggnog_df_groups.get_group(scan_list)
        sampled_table_group = sampled_table_groups.get_group(scan_list)
        ...
        if (eggnog_df_group['eggnog hmm desc'] == eggnog_df_group['eggnog hmm desc'].iloc[0]).all():
            ...
            if len(eggnog_df_group) == len(sampled_table_group):
                conserv_func_df_list.append(eggnog_df_group)
    conserv_func_df = pd.concat(conserv_func_df_list, ignore_index=True)

    # Add BLAST string annotations to DataFrame
    sampled_out_table = pd.read_csv(
        os.path.join(out_dir, 'sampled_table.csv'), header=0
        )
    sampled_out_table = sampled_out_table[['seq_number', 'stitle']]
    sampled_out_table.columns = ['seq_number', 'blast stitles']
    conserv_func_df = conserv_func_df.merge(sampled_out_table, how='left', on='seq_number')

    return conserv_func_df

def condense_hits(conserv_func_df, lower_prob_table, high_prob_taxa_table, seq_info_path):

    # Condense rows of each group of hits
    conserv_func_df = conserv_func_df.fillna('')
    scan_lists = list(set(conserv_func_df['scan_list'].tolist()))
    ...
    parsed_conserv_func_df['scan_list'] = scan_lists
    ...
    for scan_list in scan_lists:
        ...
        list_of_lists = conserv_func_df_group['kegg pathways'].apply(
            lambda x: x.split(',')
            ).tolist()
        l = [i for sublist in list_of_lists for i in sublist]
        ...
        # bigg reactions: set intersection
        list_of_lists = conserv_func_df_group['bigg reactions'].apply(
            lambda x: x.split(',')
            ).tolist()
        l = [i for sublist in list_of_lists for i in sublist]
        parsed_conserv_func_df_cols_dict['bigg reactions'].append(
            ','.join(list(set(l)))
        )
        ...
        # blast stitles: set intersection
        parsed_conserv_func_df_cols_dict['blast stitles'].append(
            '(annotation)'.join(list(set(conserv_func_df_group['blast stitles'])))
        )
    ...
    # assign taxonomic hierarchy to lower-probability sequences
    # that have been found to accord with higher-probability taxa
    lower_prob_table.set_index('scan_list', inplace=True)
    scan_lists = sorted(list(set(scan_lists).intersection(set(lower_prob_table.index.tolist()))))
    lower_prob_table = lower_prob_table.loc[scan_lists]
    lower_prob_table.reset_index(inplace=True)
    low_prob_taxa_table = assign_taxonomy(lower_prob_table, group_col='scan_list')

    if 'hit_number' in high_prob_taxa_table.columns:
        high_prob_taxa_table.drop('hit_number', axis=1, inplace=True)
    high_prob_taxa_table = assign_taxonomy(high_prob_taxa_table, group_col='scan_list')

    taxa_table = pd.concat([high_prob_taxa_table, low_prob_taxa_table], ignore_index=True)
    reported_df = parsed_conserv_func_df.merge(taxa_table, how='inner', on='scan_list')
    retained_eggnog_output_headers = []
    for header in eggnog_output_headers:
        if header not in ['query', 'evalue', 'score']:
            retained_eggnog_output_headers.append(header)
    reported_df = reported_df[
        ['scan_list'] + 
        retained_eggnog_output_headers + 
        ['blast stitles', 'best_predicts_from', 'also_contains_predicts_from'] + 
        search_ranks]
    reported_df.fillna('', inplace=True)
    reported_df.replace('NA', '', inplace=True)
    reported_df.replace('None', '', inplace=True)

    split_scan_lists = reported_df['scan_list'].apply(lambda l: l.split(',')).tolist()
    unassigned = [
        reported_df.index.tolist(),
        split_scan_lists
        ]
    assigned = [[], []]
    while unassigned[0]:
        assigned[0].append([unassigned[0][0]])
        assigned[1].append(unassigned[1][0])
        unassigned[0] = unassigned[0][1:]
        unassigned[1] = unassigned[1][1:]
        clean_pass = False
        while not clean_pass:
            clean_pass = True
            remaining_unassigned = [[], []]
            for i, row in enumerate(unassigned[0]):
                scan_list = unassigned[1][i]
                for scan in scan_list:
                    if scan in assigned[1][-1]:
                        clean_pass = False
                        assigned[0][-1].append(row)
                        assigned[1][-1] = list(set(assigned[1][-1] + scan_list))
                        break
                else:
                    remaining_unassigned[0].append(row)
                    remaining_unassigned[1].append(scan_list)
            unassigned = remaining_unassigned

    condensed_rows = []
    for group in assigned[0]:
        group_df = reported_df.loc[group]
        if (group_df['eggnog hmm desc'] == group_df['eggnog hmm desc'].iloc[0]).all():
            updated_row = []
            cols = [
                'scan_list', 
                'seed ortholog', 
            ...
            for col in cols:
                condensed_list = []
                if col == 'eggnog hmm desc':
                    pass
                elif col == 'blast stitles':
                    for i in group_df[col].tolist():
                        condensed_list += i.split('(annotation)')
                else:
                    for i in group_df[col].tolist():
                        condensed_list += i.split(',')
                if '' in condensed_list:
                    condensed_list.remove('')
                if col == 'scan_list':
                    condensed_list = list(map(str, sorted(list(set(map(int, condensed_list))))))
                else:
                    if col == 'cog cat':
                        condensed_list = sorted([i.lstrip(' ') for i in condensed_list])
                    condensed_list = sorted(list(set(condensed_list)))
                if col == 'eggnog hmm desc':
                    desc = group_df[col].iloc[0]
                    if not desc[0].isupper():
                        desc = desc[0].upper() + desc[1:]
                    updated_row.append(desc.lstrip(','))
                elif col == 'blast stitles':
                    stitles = '; '.join(condensed_list)
                    if not stitles[0].isupper():
                        stitles = stitles[0].upper() + stitles[1:]
                    updated_row.append(stitles)
                else:
                    updated_row.append(','.join(condensed_list).lstrip(','))

            for rank_index, rank in enumerate(search_ranks):
                most_common_name = Counter(group_df[rank]).most_common(1)[0]
                if most_common_name[0] != '' and pd.notnull(most_common_name[0]):
                    if most_common_name[1] == len(group_df):
                        updated_row += rank_index * [''] + group_df.iloc[0][rank:].tolist()
                        break
            else:
                updated_row += len(search_ranks) * ['']

            condensed_rows.append(updated_row)

    reported_df = pd.DataFrame(condensed_rows, columns=reported_df.columns)
    # Add a column of counts of scans in each scan list
    reported_df['scan count'] = [
        scan_list.count(',') + 1 for scan_list in reported_df['scan_list'].tolist()
        ]
    reordered_cols = reported_df.columns.tolist()
    reordered_cols = reordered_cols[:1] + reordered_cols[-1:] + reordered_cols[1:-1]
    reported_df = reported_df[reordered_cols]
    reported_df.sort_values(by=['cog cat', 'eggnog hmm desc', 'blast stitles'], inplace=True)
