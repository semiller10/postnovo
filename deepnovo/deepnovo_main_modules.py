  ...
  for location in spectra_locations:
    ...
    if data_format == "mgf":
      ...
      if peptide_mass > deepnovo_config.MZ_MAX:
        print('skipped mass=' + str(peptide_mass))
        print('skipped mass scan=' + str(scan))
        counter_skipped += 1
...
def print_AA_basic(output_file_handle,
  ...
  # output_seq, output_score, output_aa_scores = output
  ...
  output_score = 10 ** output_score
  output_aa_scores = [10 ** score for score in output_aa_scores]
  aa_scores = ' '.join(map(str, [round(s, 2) for s in output_aa_scores]))

  print ("%s\t%s\t%s\t%.2f\t%s\t%d\t%d\t%d\n"
         % (scan,
...
def test_AA_decode_batch(scans,
  ...
  max_reported_seqs = deepnovo_config.FLAGS.beam_size
  ...
  #~ for index in xrange(15,20):
    ...
    scan_outputs = outputs[index]

    reported_seqs = []
    for output in scan_outputs:
      if len(reported_seqs) < max_reported_seqs:
        output_seq, _, _ = output

        if output_seq != deepnovo_config.EOS_ID:
          ...
          trimmed_decoder_input = trim_decoder_input(decoder_input, direction)
          decoder_input_AA = [deepnovo_config.vocab_reverse[x] for x in trimmed_decoder_input]
          ...
          if output_AA not in reported_seqs:
            ...
            print_AA_basic(output_file_handle,
                          ...
                          len_AA,
                          exact_match)
            reported_seqs.append(output_AA)
...
def decode_beam_select_2(output_top_paths):
  ...
  for top_paths in output_top_paths:
    ...
    if not top_paths: # cannot find the peptide
      ...
      # output_score = float("inf")
      output_seqs = [deepnovo_config.EOS_ID]
      output_scores = [float("inf")]
      output_aa_scores = [[float("inf")]]
      #~ print("ERROR: no path found ", entry)
      #~ sys.exit()
    else:
      # path format: [path, score, direction, aa_scores]
      # path_scores = np.array([x[1] for x in top_paths])
      # path_lengths = np.array([len(x[0]) for x in top_paths])
      path_avg_score_duples = list(
        zip([path[1]/len(path[0]) for path in top_paths],
        list(range(len(top_paths))))
        )
      # print(path_avg_score_duples)
      path_avg_score_duples.sort(key=lambda duple: -duple[0])
      # print(path_avg_score_duples)
      #~ top_path = top_paths[np.argmax(path_scores)]
      # top_path = top_paths[np.argmax(path_scores/path_lengths)]
      top_paths = [top_paths[path_avg_score_duples[i][1]] for i in range(len(path_avg_score_duples))]
      # print(top_paths)
      # output_seq = top_path[0]
      output_seqs = [path[0] + [deepnovo_config.EOS_ID] for path in top_paths]
      # print(output_seqs)
      # output_score = top_path[1] / len(output_seq)
      output_scores = [path[1]/len(path[0]) for path in top_paths]
      # print(output_scores)
      # output_seq.append(deepnovo_config.EOS_ID)
      #~ output_score = top_path[1]
      output_aa_scores = [path[2] for path in top_paths]
      # print(output_aa_scores)

    # output sequence with score
    # outputs.append([output_seq, output_score, output_aa_scores])
    outputs += [[[output_seqs[i], output_scores[i], output_aa_scores[i]] for i in range(len(output_seqs))]]
...
def decode_beam_search_01(sess,
  ...
  search_len = min([decode_block_size, len(prefix_mass_list)])
  # for spectrum_id in xrange(decode_block_size):
  for spectrum_id in xrange(search_len):
      ...
      for path in current_paths:
        ...
        if AA_ID_2 == LAST_LABEL: # nobi
          if (abs(prefix_mass - peptide_mass)
            ...
            output_top_paths[spectrum_id].append([path[0], path[2], direction, path[5]])
          continue
        ...
        block_aa_scores.append(aa_scores)
    ...
    for entry_index, entry in enumerate(active_search):
      ...
      for index in xrange(block_index,
        ...
        for aa_id in block_mass_filter_candidate[index]:
          ...
          if aa_id > 2:
            new_paths[-1].append(block_aa_scores[index] + [current_log_prob[index][aa_id]])
          else:
            new_paths[-1].append(block_aa_scores[index])
      ...
      if ((not top_paths_forward[spectrum_id])
        ...
        for x_path in top_paths_forward[spectrum_id]:
          for y_path in top_paths_backward[spectrum_id]:
            ...
            # print('seq forward: ' + ' '.join(map(str, seq_forward)))
            seq_backward = y_path[0][1:-1]
            # print('seq backward: ' + ' '.join(map(str, seq_backward)))
            seq_backward = seq_backward[::-1]
            seq = seq_backward + seq_forward
            # print('seq: ' + ' '.join(map(str, seq)))
            score = x_path[1] + y_path[1]
            # print('score: ' + str(score))
            aa_scores = x_path[3] + y_path[3][::-1]
            # print('aa scores: ' + ' '.join(map(str, aa_scores)))

            output_top_paths[spectrum_id].append([seq, score, aa_scores])
...
def decode_beam_search(sess,
  ...
  if deepnovo_config.FLAGS.direction == 0 or deepnovo_config.FLAGS.direction == 1:
    ...
    #                                        deepnovo_config.FLAGS.direction,
    #                                        output_file_handle)
    test_AA_decode_batch(
      scans, decoder_inputs, outputs, deepnovo_config.FLAGS.direction, output_file_handle
    )
  else:
    ...
    #                                        output_file_handle)
    test_AA_decode_batch(
      scans, decoder_inputs_forward, outputs, 0, output_file_handle
      )
...
#~     test_writer.close()
    ...
    if deepnovo_config.FLAGS.beam_search:
      ...
      with open(decode_output_file, 'w') as output_file_handle:
        print("scan\ttarget_seq\toutput_seq\tseq_probability\tAA_probability\taccuracy_AA\tlen_AA"
              "\texact_match\n",
              file=output_file_handle,
              end="")
        ...
        with open(input_file, 'r') as input_file_handle:
          ...
          for stack in spectra_file_location_stack_list:
            ...
            #                                      output_file_handle)
            decode_beam_search(
              sess, model, stack_data_set, knapsack_matrix, output_file_handle
              )
