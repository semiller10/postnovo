...
do
	...
	nohup $BLASTP_PATH -task blastp-short -db $DB_DIR -query $fasta_file -out $out_file -evalue 1000000 -max_target_seqs 500 -comp_based_stats 0 -outfmt "6 qseqid sgi sacc qstart qend sstart send evalue bitscore pident staxids" &
	process_ids=(`pgrep -u $(id -u -n) blastp`)
done
