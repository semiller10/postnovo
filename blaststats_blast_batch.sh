...
do
	...
	nohup $BLASTP_PATH -db $DB_DIR -query $fasta_file -out $out_file -evalue 1000000 -max_target_seqs 1 -max_hsps 1 -comp_based_stats 0 -outfmt "6 qseqid sacc qstart qend sstart send evalue bitscore pident staxids" &
	process_ids=(`pgrep -u $(id -u -n) blastp`)
done
